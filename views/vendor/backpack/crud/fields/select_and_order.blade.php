...
@include('crud::fields.inc.wrapper_start')
    ...
    <div class="row mb-1">
        <div class="col-12">
            <input type="text" class="form-control {{ $field['name'] }}_dragFilter" placeholder="Filter">
        </div>
    </div>
    <div class="row"
        ...
        <div class="col-md-12">
            ...
            <ul data-identifier="drag-source" class="{{ $field['name'] }}_connectedSortable {{ $field['name'] }}_sourcelist select_and_order_all float-right"></ul>

            {{-- The results will be stored here --}}
            <div data-identifier="results">
                <select class="d-none" 
                    name="{{ $field['name'] }}[]" 
                    data-selected-options='{{ implode(",", $values) }}'
                    multiple>
                </select>
            </div>
        </div>
...
<script>
    function bpFieldInitSelectAndOrderElement(element) {
        ...
        $("."+$fieldName+"_dragFilter").keyup(function(){
            var valThis = $(this).val().toLowerCase();
            
            $("ul."+$fieldName+"_sourcelist>li").each(function(){
